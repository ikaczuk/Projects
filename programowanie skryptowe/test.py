...
def gra():
    ...
    for i in range(9):
        if tura == "kółko" and tablica[kolumna][wiersz] == "0":
            tablica[kolumna][wiersz] == "o"
            figura = czcionka.render("o", True, pg.Color("blue"))
            screen.blit(figura, (kolumna * 245, wiersz * 245))
            pg.display.update()
            tura = "krzyżyk"
            return
        elif tura == "krzyżyk" and tablica[kolumna][wiersz] == "0":
            tablica[kolumna][wiersz] == "x"
            figura = czcionka.render("x", True, pg.Color("blue"))
            screen.blit(figura, (kolumna * 245, wiersz * 245))
            pg.display.update()
            tura = "kółko"
            return
...
def czy_wygrana():
    if tablica[0][0]==tablica[1][1]==tablica[2][2]=="x" or tablica[2][0]==tablica[1][1]==tablica[0][2]=="x":
        print("WYGRYWA x!")
    elif tablica[0][0]==tablica[1][1]==tablica[2][2]=="o" or tablica[2][0]==tablica[1][1]==tablica[0][2]=="o":
        print("WYGRYWA o!")
    else:
        for i in range(3):
